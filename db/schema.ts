...
export const trademarkSearches = pgTable("trademark_searches", {
  ...
  classes: jsonb("classes").$type<number[]>().default([]),
  ...
  streamedConflicts: jsonb("streamed_conflicts").$type<Record<string, unknown>[]>().default([]),
  expertAnalysis: jsonb("expert_analysis").$type<Record<string, unknown>>(),
  nameShortlist: jsonb("name_shortlist").$type<string[]>().default([]),
